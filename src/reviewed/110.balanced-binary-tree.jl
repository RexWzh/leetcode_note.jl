...
function is_balanced_binary_tree(root::TreeNode)::Bool
    ...
    function tree_depth(tree::TreeNode, depth::Int = 0)::Int
...
@testset "110.balanced-binary-tree.jl" begin
    @test is_balanced_binary_tree(TreeNode{Int}([3,9,20,nothing,nothing,15,7])) == true
    @test is_balanced_binary_tree(TreeNode{Int}([1,2,2,3,3,nothing,nothing,4,4])) == false
    @test is_balanced_binary_tree(nothing) == true
end
## @lc test=end

## @lc info=start
# link: [solution to 110](https://leetcode-cn.com/problems/balanced-binary-tree/solution/110-ping-heng-er-cha-shu-by-rex-rs-qvmg/)
## @lc info=end